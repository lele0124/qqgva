...
func main() {
	...
	// 首次运行时，扫描已有的文件
	scanAndProcessExistingFiles(dialogueDir, processedFiles)
	...
	for {
		select {
		case <-ticker.C:
			scanNewFiles(dialogueDir, processedFiles)
		}
	}
}

// 扫描并处理已有的文件
func scanAndProcessExistingFiles(dir string, processedFiles map[string]bool) {
...
// 扫描新文件
func scanNewFiles(dir string, processedFiles map[string]bool) {
	files, err := ioutil.ReadDir(dir)
	if err != nil {
		log.Printf("读取目录失败: %v\n", err)
		return
...
func processFile(filePath string) {
	...
	if fileName == "README.md" || strings.HasPrefix(fileName, "start_") || strings.HasSuffix(fileName, ".go") {
		return
	}
	...
	if summary != "" {
		updateProjectRecord(summary, files, filePath)
	}
}
...
// 从记录文件中加载已处理的文件信息
func loadProcessedFiles(processedFiles map[string]bool) {
	content, err := ioutil.ReadFile(projectRecordFile)
	if err != nil {
		// 文件不存在，不需要加载
		return
	}
...
// 为了测试，创建一个简单的示例文件生成函数
func createExampleDialogueFile() {
...
4. 可以尝试重启服务或者清除缓存`
	...
	err := ioutil.WriteFile(exampleFilePath, []byte(exampleContent), 0644)
	if err != nil {
